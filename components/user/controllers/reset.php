        ...
        public function reset()
        {
            if (Core::config()->enable_recaptcha == 1)
            {
                ...
                else
                {
                    ...
                    header("Location: ". Core::route("index.php?component=user&controller=reset"));
                }
            }
            else
            {
        ...
        public function processReset()
        {
            ...
            if ($reset->id > 0)
            {
                if ($_POST["password"] == $_POST["password_again"])
                {
                    ...
                    $this->model->database->query("DELETE FROM #__users_recovery WHERE token = ?", array($_POST["token"]));
                ...
                else
                {
                    ...
                    header("Location: ". Core::route("index.php?component=user&controller=reset&token=". $_POST["token"]));
                }
            }
            else
            {
